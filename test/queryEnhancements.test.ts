...
import { ReadQL } from '../src/crud/read.ts'
import { GroupByQueryBuilder } from '../src/capabilities/groupBy.ts'
import { HavingQueryBuilder } from '../src/capabilities/having.ts'
import { Op } from '../src/crud/base.ts'
...
// Enhanced test types for aggregation queries
...
// Test implementation for GroupBy capabilities
...
// Test implementation for Having capabilities
	...
	describe('having()', () => {
		...
		it('should reject dangerous HAVING conditions', () => {
			...
			assertThrows(
				...
				Error,
				'Potentially dangerous SQL pattern detected in HAVING condition'
			)
		})
		...
		it('should reject overly long HAVING conditions', () => {
			...
			assertThrows(
				...
				Error,
				'HAVING condition cannot exceed 1000 characters'
			)
		})
...
describe('Enhanced pagination support', () => {
	describe('offset and limit combinations', () => {
		it('should support enhanced pagination with large datasets', async () => {
			...
			const mockData = Array.from({ length: 10 }, (_, i) => createTestUserRaw({
